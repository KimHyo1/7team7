...
const emit = defineEmits(['submit', 'cancel', 'update:form', 'place-selected'])
...
function onSubmit() {
  // 선택된 장소 이미지가 있으면 로컬 폼에 반영
  if (selectedPlace.value && selectedPlace.value.image) {
    localForm.thumbnail = selectedPlace.value.image;
  }
  // 부모에 최신 폼을 전달한 뒤 제출 이벤트 발생
